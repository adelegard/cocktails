class RecipeUser < ActiveRecord::Base
	...
	validates :starred, :inclusion => {:in => [true, false]}, :allow_nil => true
	validates :liked, :inclusion => {:in => [true, false]}, :allow_nil => true
	...
	class << self
		...
		def set_note(recipe_id, user_id, note)
	    recipe_user = RecipeUser.find_or_initialize_by_recipe_id_and_user_id(recipe_id, user_id)
	    recipe_user.note = note
	    recipe_user.note_updated_at = Time.now
	    recipe_user.save
		end
